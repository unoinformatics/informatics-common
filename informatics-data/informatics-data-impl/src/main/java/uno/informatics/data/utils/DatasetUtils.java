...
public class DatasetUtils
{
	...
	public static final List<ColumnFeature> generateDatasetFeatures(
			FileProperties fileProperties, String columnLabel) throws DatasetException
	{
		return generateDatasetFeatures(fileProperties, columnLabel, INVALID_ROW_COUNT) ;
	}
	
	public static final List<ColumnFeature> generateDatasetFeatures(
			FileProperties fileProperties, String columnLabel, int numRows) throws DatasetException
	{
		...
		if (fileProperties.getRowHeaderPosition() > UNKNOWN_INDEX && 
			...
			throw new DatasetException("Header position : " + 
					fileProperties.getDataRowPosition() + " must be before data position : " + fileProperties.getRowHeaderPosition() ) ;
			...
			if (reader != null && reader.ready())
			{
				...
				if (fileProperties.hasRowHeader())
				{
					...
					else
					{
						...
						{
							if (headers.size() != dataTypes.size())
								throw new DatasetException("Number of headers : " + headers.size() + " does not match number of columns : " + dataTypes.size()) ;
						}
					}
				}
				else
