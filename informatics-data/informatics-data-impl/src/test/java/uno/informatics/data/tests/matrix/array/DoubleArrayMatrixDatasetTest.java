...
public class DoubleArrayMatrixDatasetTest
{
	...
	private static final String ROW_HEADERS_NAME = "rowheaders";
	private static final String COLUMN_HEADERS_NAME = "columnheaders";
	private static final Object[] ROW_HEADERS = new Object[] {"R1", "R2", "R3"};
	private static final Object[] COLUMN_HEADERS = new Object[] {"C1", "C2", "C3", "C4", "C5"};

	/**
	 * Test method for {@link uno.informatics.data.matrix.array.DoubleArrayMatrixDataset#createMatrixDataset(java.lang.String, java.lang.String, java.lang.String, uno.informatics.common.model.Feature, uno.informatics.common.io.FileProperties, uno.informatics.common.model.Feature, uno.informatics.common.model.Feature)}.
	 */
	@Test
	public void testCreateMatrixDataset()
	{
		FileProperties fileProperties = new FileProperties(DoubleArrayMatrixDatasetTest.class.getResource(DATA_FILE1).getPath(), FileType.TXT) ;
	  
	  testCreateMatrixDataset(fileProperties) ;
	  
	  fileProperties = new FileProperties(DoubleArrayMatrixDatasetTest.class.getResource(DATA_FILE2).getPath(), FileType.TXT) ;
	  ...
	  fileProperties = new FileProperties(DoubleArrayMatrixDatasetTest.class.getResource(DATA_FILE3).getPath(), FileType.TXT) ;
    
	  fileProperties.setRowHeaderPosition(0) ;
	  
	  testCreateMatrixDataset(fileProperties) ;
	  
	  fileProperties = new FileProperties(DoubleArrayMatrixDatasetTest.class.getResource(DATA_FILE4).getPath(), FileType.TXT) ;
	...
	public void testCreateMatrixDataset(FileProperties fileProperties)
	{
		try
    {
	    Feature elementFeature = new SimpleFeaturePojo(ELEMENT_NAME, DataType.STRING, ScaleType.NOMINAL)  ;
			Feature rowHeaderFeature = new SimpleFeaturePojo(ROW_HEADERS_NAME, DataType.STRING, ScaleType.NOMINAL)  ;
			Feature columnHeaderFeature = new SimpleFeaturePojo(COLUMN_HEADERS_NAME, DataType.STRING, ScaleType.NOMINAL)  ;
			
			MatrixDataset<Double> matrix = 
	    		DoubleArrayMatrixDataset.createMatrixDataset(
	    				UID, NAME, DESCRIPTION, elementFeature, 
	    				fileProperties, rowHeaderFeature, columnHeaderFeature);
	    ...
	    if (matrix.hasColumnHeaders())
			...
			for (int x = 0 ; x < matrix.getRowCount() ; ++x)
				for (int y = 0 ; y < matrix.getColumnCount() ; ++y)
					assertEquals("x="+x+" y="+y, new Double(""+(x+1)+"."+(y+1)), (double)matrix.getValue(x, y), DELTA) ;
